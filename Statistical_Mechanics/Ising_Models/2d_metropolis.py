...
def run_matplotlib(N, epsilon, steps, J, T):
    ...
    axes[0].scatter(x.flatten(), y.flatten(), c=ini_grid.flatten(), cmap="bwr")
    axes[0].set_aspect("equal")
    axes[0].grid(True)
    axes[0].set_title("Initial")

    axes[1].scatter(x.flatten(), y.flatten(), c=grid.flatten(), cmap="bwr")
    axes[1].set_aspect("equal")
    axes[1].grid(True)
    axes[1].set_title("Final")

    axes[2].imshow(ini_grid, cmap="bwr", interpolation="bicubic", origin="lower")
    axes[2].set_title("Initial Heat Map")

    axes[3].imshow(grid, cmap="bwr", interpolation="bicubic", origin="lower")
